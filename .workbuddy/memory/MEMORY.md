...
## 核查报告问题的复核结论（`build/audit_issues.py`）
该脚本把核查报告的 8 项问题做成**可复跑回归检查**，用法：先 `python build/build.py` 再 `python build/audit_issues.py`。
- 问题 1/2/3/4/6/7/8 **已全部解决**；问题 5 **部分解决**。
- 问题 5 唯一残留：湖北武汉（114.30°E）在孝感（113.93°E）以东，却归"以西"的江汉 ——
  与面板"江汉=江淮、黄淮以西"的表述相悖。**属已声明的推断性细化，页脚有披露，非错误**。
- 问题 5 的福建项**原判断有误已撤回**（三明 26.27°N > 福州 26.07°N，归江南/华南本就自洽）。
- 架构与代码级问题 10/10 已处理（含 TLS 校验已恢复、调试文件已清理、键盘可达已补）。
- 结论：除"口径属推断"这类**已披露的局限**外，核查报告提出的问题均已闭环。

## 核查报告自身的维护注意
`核查报告-中国气象地理区划交互地图.html` **不是自动生成**，是手写文档，改项目后需人工同步。
已知易失步点（2026-09-15 已修一轮）：
- **体积数字**必须与实际一致。当前 **864.8 KB**（内嵌数据 824.6 KB 占 95.4%）。历史沿革：857.6（核查时）→ 863.8（二轮修复）→ 864.8（补充文档与断言后）。
- **不要声称过程文件"已归档可回溯"**。`build/dev-scratch/`、`build/shots/`、`build/audit_shots/` 均已删除，
  报告中的引用须标注为"历史快照"。`build/audit_shots/` 是 `audit_render.js` 的输出目录，按需生成、已加入 .gitignore。
- 报告结构：头部 meta（含文档说明）→ 修复完成情况表 → 一、核查范围与方法 → 二、数据真实性核查（含 2.3 发现的问题 8 项）
  → 三、代码及方案评估（3.1–3.4）→ 四、值得肯定的方面 → 五、核查局限 → footer（核查方法可复现）。
- 校验方法：`grep -c '857.6\|863.8'` 看是否残留过时值；标签配对用正则统计开闭标签数；
  渲染验证看 `pageerror`/`console.error` 是否为零（实测零错误）。
